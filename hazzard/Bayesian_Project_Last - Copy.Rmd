...
df$GradeClass <- as.factor(df$GradeClass)  # This is the target variable, so we exclude it

# Normalize numerical variables (optional)
df_norm <- df %>%
  mutate(across(c(StudyTimeWeekly, Absences, GPA), scale))

# Remove non-informative columns (StudentID, GradeClass)
df_cluster <- df_norm %>% select(-c(StudentID, GradeClass))
...
# Apply K-Means with k = 3 (as an example)
...
# Remove non-informative columns and scale continuous variables
df_cluster <- data %>%
  select(GPA, StudyTimeWeekly, Absences, ParentalSupport, Tutoring) %>%
  mutate(across(where(is.numeric), scale))  # Standardize numerical columns

```

```{r}
set.seed(123)
fviz_nbclust(df_cluster, kmeans, method = "wss")  # Elbow method
...
# Cut into 3 clusters
data$Cluster_HC <- cutree(hc, k = 3)

```
```{r}
gmm_result <- Mclust(df_cluster)

# Summary of GMM clustering
summary(gmm_result)

# Assign cluster labels
data$Cluster_GMM <- as.factor(gmm_result$classification)

# Visualization
fviz_cluster(list(data = df_cluster, cluster = gmm_result$classification))

```

```{r}
aggregate(GPA ~ Cluster_KMeans, data, mean)
aggregate(StudyTimeWeekly ~ Cluster_KMeans, data, mean)
aggregate(Absences ~ Cluster_KMeans, data, mean)
...
data$Cluster_HC <- as.factor(cutree(hc, k = 3))
...
# Define Bayesian Ordinal Model formula using Cluster_HC
formula = GradeClass ~ Gender + ParentalEducation + ParentalSupport + 
          StudyTimeWeekly + Absences + Tutoring + Ethnicity + (1 | Cluster_HC)

# Define priors
priors <- c(
  set_prior("normal(0, 10)", class = "b"),  # Broad prior for regression coefficients
  set_prior("cauchy(0, 1)", class = "sd"),  # Prior for random effect standard deviation
  set_prior("student_t(3, 0, 10)", class = "Intercept")  # Weakly informative prior for intercept
)

# Fit the Bayesian ordinal model
mdl_ord_hc <- brm(
  formula = formula,
...
# Plot Bayesian model coefficients
plot(mdl_ord_hc)
